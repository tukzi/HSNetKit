...
+ (NSString *)YKNetworking_md5:(NSString *)string;
...
static inline NSString *cachePath() {
    return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/YKNetworkingCaches"];
}
...
+ (void)clearCaches {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:directoryPath isDirectory:nil]) {
        ...
        if (error) {
            NSLog(@"YKNetworking clear caches error: %@", error);
        } else {
            NSLog(@"YKNetworking clear caches ok");
        }
    }
}
...
+ (HYBURLSessionTask *)getWithHost:(NSString *)host
                           ...
                           success:(HYBResponseSuccess)success
                              fail:(HYBResponseFail)fail{
    return   [self getWithUrl:[NSString stringWithFormat:@"%@%@",host,url]
                 refreshCache:NO
                       params:params
                     progress:nil
                      success:success
...
+ (HYBURLSessionTask *)postWithHost:(NSString *)host
                            ...
                            success:(HYBResponseSuccess)success
                               fail:(HYBResponseFail)fail{
    return   [self postWithUrl:[NSString stringWithFormat:@"%@%@",host,url]
                  refreshCache:NO
                        params:params
                      progress:nil
                       success:success
        ...
        if (sg_cacheGet) {
            if (sg_shoulObtainLocalWhenUnconnected) {
                if (sg_networkStatus == kHYBNetworkStatusNotReachable ||  sg_networkStatus == kHYBNetworkStatusUnknown ) {
                    id response = [YKNetWorking cahceResponseWithURL:absolute
                                                          parameters:params];
                    if (response) {
                        if (success) {
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        NSString *key = [NSString YKNetworking_md5:absoluteURL];
